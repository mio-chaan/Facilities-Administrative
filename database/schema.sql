...
CREATE TABLE team8_records (
    ...
    status          VARCHAR(30) NOT NULL DEFAULT 'active', -- active | due_review | archived | pending_disposal | disposed
    archived_at     DATETIME NULL,
    archive_reason  VARCHAR(500) NULL,
    disposed_at     DATETIME NULL,
    disposal_reason VARCHAR(500) NULL,
    disposal_requested_by  INT NULL,
    disposal_requested_at  DATETIME NULL,
    disposal_authorized_by INT NULL,
    disposal_authorized_at DATETIME NULL,
    created_at      DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at      DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted_at      DATETIME NULL,
    ...
    CONSTRAINT uq_team8_records_entity UNIQUE (entity_type, entity_id)
) ENGINE=InnoDB;
...
CREATE INDEX idx_team8_memo_recipient_department ON team8_memorandum_recipients (department_id);
...
CREATE INDEX idx_team8_reservations_department ON team8_reservations(department_id);
...
CREATE INDEX idx_team8_documents_browse_owner ON team8_documents(uploaded_by, status, deleted_at, updated_at, id);
CREATE INDEX idx_team8_documents_browse_category ON team8_documents(category_id, status, deleted_at, updated_at, id);
CREATE INDEX idx_team8_documents_browse_deleted ON team8_documents(deleted_at, updated_at, id);
CREATE INDEX idx_team8_ir_browse ON team8_incident_reports(status, employee_id, updated_at, id);
CREATE INDEX idx_team8_nte_browse ON team8_notice_to_explain(status, employee_id, updated_at, id);
CREATE INDEX idx_team8_expl_browse ON team8_explanations(status, employee_id, updated_at, id);
CREATE INDEX idx_team8_memo_browse ON team8_memorandums(status, kind, updated_at, id);
CREATE INDEX idx_team8_memo_recipient_browse ON team8_memorandum_recipients(memorandum_id, department_id, recipient_type);
CREATE INDEX idx_team8_cert_browse ON team8_certificates(status, employee_id, updated_at, id);
